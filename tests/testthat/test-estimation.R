...
test_that("preprocessed example data includes required matrices", {
  data("osu_surveillance_preprocessed", package = "PrevKalman")

  expect_true(is.list(osu_surveillance_preprocessed))
  expect_true(all(c("D", "Y", "R", "Z_next", "C_t", "Sympt_t", "gamma", "days") %in%
    names(osu_surveillance_preprocessed)))
  expect_equal(ncol(osu_surveillance_preprocessed$D), length(osu_surveillance_preprocessed$days))
  expect_true(all(osu_surveillance_preprocessed$test_counts >= 0))
})
