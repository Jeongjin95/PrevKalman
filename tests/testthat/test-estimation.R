...
test_that("prepare_surveillance_data builds expected matrices", {
  dat <- data.frame(
    name_n = c(1, 1, 2, 2),
    test_day = c(1, 3, 1, 2),
    provider = c(1, 1, 1, 1),
  ...
  prep <- prepare_surveillance_data(
    ...
    clearance_days = 1,
    exempt_days = 1,
    symptom_unobserved_days = 1,
    low_test_threshold = 1
  )

  expect_equal(dim(prep$D), c(2, 4))
  expect_equal(dim(prep$Y), c(2, 4))
  expect_equal(length(prep$gamma) > 0, TRUE)
  expect_true(all(prep$test_counts >= 0))
})
