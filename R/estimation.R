...
#'
#' @param prepared_data Optional output from [prepare_surveillance_data()].
#' @param D Binary testing indicator matrix.
#' @param Y Binary test-result matrix.
#' @param R Logical removed-state matrix.
#' @param Z_next Matrix of next testing times.
...
#'   `seq_len(ncol(D))`.
#'
#' @return A list with `results`, `estimate`, `se`, `lower`, `upper`, and the
#'   full `jackknife` output.
#' @export
#'
#' @examples
#' sim <- generate_surveillance_data(
...
ht_prevalence <- function(
  ...
  list(
    ...
    jackknife = jk_out,
    days = inputs$days,
    low_test_days = if (!is.null(prepared_data$low_test_days)) prepared_data$low_test_days else integer(0),
...
#' Prepare Long-Form Repeated-Testing Data
#'
#' Converts long-form repeated-testing data into the matrix representation used
#' by the Horvitz-Thompson and Kalman procedures.
#'
#' @param data A data frame in long format.
#' @param id_col Column giving the individual identifier.
#' @param day_col Column giving the study day.
#' @param result_col Column giving the binary test result.
#' @param symptom_col Column giving the binary symptom/contact indicator.
#' @param days Optional vector of days to include. Defaults to
#'   `seq_len(max(data[[day_col]]))`.
#' @param clearance_days Number of clearance days after a positive test.
#' @param exempt_days Additional days exempt from testing after clearance.
#' @param symptom_unobserved_days Integer days whose symptom indicator should
#'   be forced to zero.
#' @param low_test_threshold Optional threshold used to flag low-test days.
#'
#' @return A list containing prepared matrices and metadata.
#' @export
#'
#' @examples
...
#' )
#' names(prep)
prepare_surveillance_data <- function(
    data,
    id_col = "name_n",
    day_col = "test_day",
    result_col = "result",
    symptom_col = "symp_cont",
    days = NULL,
    clearance_days = 10,
    exempt_days = 80,
    symptom_unobserved_days = integer(0),
    low_test_threshold = NULL) {
  if (!is.data.frame(data)) {
    stop("`data` must be a data frame.")
  }

  if (is.null(days)) {
    days <- seq_len(max(data[[day_col]], na.rm = TRUE))
  }

  ids <- sort(unique(data[[id_col]]))
  max_days <- length(days)

  D_full <- matrix(
    0L,
    nrow = length(ids),
    ncol = max_days,
    dimnames = list(as.character(ids), as.character(days))
  )
  Y_full <- D_full
  A_sympt_full <- D_full

  row_index <- match(data[[id_col]], ids)
  col_index <- match(data[[day_col]], days)
  keep <- !is.na(row_index) & !is.na(col_index)

  D_full[cbind(row_index[keep], col_index[keep])] <- 1L
  Y_full[cbind(row_index[keep], col_index[keep])] <- as.integer(data[[result_col]][keep])
  A_sympt_full[cbind(row_index[keep], col_index[keep])] <- as.integer(data[[symptom_col]][keep])

  D <- D_full
  Y <- Y_full
  A_sympt <- A_sympt_full

  symptom_idx <- match(symptom_unobserved_days, days, nomatch = 0L)
  symptom_idx <- symptom_idx[symptom_idx > 0]
  if (length(symptom_idx) > 0) {
    A_sympt[, symptom_idx] <- 0L
  }

  total_removed_days <- clearance_days + exempt_days

  for (i in seq_len(nrow(D))) {
    pos_days <- which(Y[i, ] == 1)
    for (t in pos_days) {
      if (t + 1 <= max_days) {
        end_removed <- min(t + total_removed_days, max_days)
        D[i, (t + 1):end_removed] <- 0L
        Y[i, (t + 1):end_removed] <- 0L
        A_sympt[i, (t + 1):end_removed] <- 0L
      }
    }
  }

  Z_next <- matrix(NA_real_, nrow = nrow(D), ncol = ncol(D))
  Z_next[, max_days] <- ifelse(D[, max_days] == 1, max_days, Inf)

  if (max_days > 1) {
    for (t in seq.int(max_days - 1, 1)) {
      Z_next[, t] <- ifelse(D[, t] == 1, t, Z_next[, t + 1])
    }
  }

  Sympt_t <- matrix(0L, nrow = nrow(D), ncol = ncol(D))
  for (i in seq_len(nrow(D))) {
    last_time <- 0L
    for (t in seq_len(ncol(D))) {
      if (isTRUE(A_sympt[i, t] == 1L)) {
        last_time <- t
      }
      Sympt_t[i, t] <- last_time
    }
  ...
  R <- matrix(FALSE, nrow = nrow(D), ncol = max_days)
  C_t <- matrix(0L, nrow = nrow(D), ncol = max_days)

  for (t in seq_len(max_days)) {
    tested_positive <- Y[, t] == 1L
    ...
    if (t > total_removed_days && t < max_days) {
      cleared_today <- rowMeans(
        R[, (t - total_removed_days + 1):t, drop = FALSE]
      ) == 1

      R[cleared_today, t + 1] <- FALSE
      C_t[cleared_today, t + 1] <- t
    }

    if (t < max_days) {
      C_t[, t + 1] <- pmax(C_t[, t + 1], C_t[, t])
    }
  }

  gamma <- unique(as.vector(C_t))
  low_test_days <- integer(0)
  if (!is.null(low_test_threshold)) {
    low_test_days <- which(colSums(D) < low_test_threshold)
  }
  ...
  list(
    ...
    gamma = gamma,
    days = days,
    low_test_days = low_test_days,
    test_counts = colSums(D),
    total_removed_days = total_removed_days
  )
}

#' Estimate the Full HT + Kalman Prevalence Pipeline
#'
#' Runs the manuscript's main repeated-testing workflow from prepared matrices:
#' Horvitz-Thompson estimation, block jackknife uncertainty quantification,
#' missing-day masking, and Kalman model fitting.
#'
#' @param prepared_data Output from `prepare_surveillance_data`.
#' @param spec Test specificity.
#' @param sens Test sensitivity.
#' @param n_blocks Number of jackknife blocks.
#' @param conf_level Confidence level for the jackknife intervals.
#' @param seed Random seed for block assignment.
#' @param ci_method Confidence-interval option passed to
#'   [fit_kalman_model()].
#' @param low_test_days Optional vector of days to mask as missing. Defaults to
#'   `prepared_data$low_test_days`.
#' @param n_cores Number of cores used by `block_jackknife`.
#'
#' @return A list containing the HT estimates and the fitted joint Kalman
#'   results.
#' @export
#'
#' @examples
...
#' )
#'
#' fit <- estimate_prevalence_pipeline(
#'   prep,
#'   spec = 1,
#'   sens = 0.832,
#'   n_blocks = 5,
...
#' )
#' }
estimate_prevalence_pipeline <- function(
    prepared_data,
    spec,
    sens,
    n_blocks = 20,
    ...
    seed = 12345,
    ci_method = c("model_based", "ht_se", "ht_se_adjusted", "all"),
    low_test_days = prepared_data$low_test_days,
    n_cores = safe_mc_cores()) {
  required_names <- c("D", "Y", "R", "Z_next", "C_t", "Sympt_t", "gamma", "days")
  missing_names <- setdiff(required_names, names(prepared_data))
  if (length(missing_names) > 0) {
    stop("`prepared_data` is missing required elements: ", paste(missing_names, collapse = ", "))
  }
  ...
  if (length(low_test_days) > 0) {
    ht$estimate[low_test_days] <- NA_real_
    ht$se[low_test_days] <- NA_real_
    ht$lower[low_test_days] <- NA_real_
    ht$upper[low_test_days] <- NA_real_
  }
  ...
  list(
    ...
    ht_fit = ht_fit,
    kalman = kalman_fit,
    joint_fit = kalman_fit$joint_fit,
    jackknife = ht_fit$jackknife,
    prepared_data = prepared_data,
    low_test_days = low_test_days
  )
}
